...
enum {<(1)write state list>};
//enum {C = 1, G, A, B, H, D, F, E};
string stateName[] = {<(2)write stateString list>}; // for test print
//string stateName[] = {"", "C", "G", "A", "B", "H", "D", "F", "E"}; // for test print
#define STATES_SIZE <(3)write state size>
//#define STATES_SIZE 8
extern <(4)write Condition list, for future work>;
//extern Event1, Event2, Event3;
extern <(5)write Action list, for future work>;
//extern Action1();
//extern Action2();
//extern Action3();
...
BinTree<int>* EFSM::CreateBinTree(int stateId, BinTree<int> *aParent) { // create BinTree for the default states
    ...
    switch(stateId) {
		case <(6)write state>:
			<if (it's first thread)> {
				s->left = CreateBinTree(<write sub first thread>, s);
				<if (it's parent has second thread)>
					s->right = CreateBinTree(<write sub second thread>, s->parent);
			}
			<else>
			s->left = CreateBinTree(<write first substate(default substate)>, s);
        /*case E:
...
void EFSM::Run() { // input event and change ActiveStates
	...
	while (!it.IsDone()) {
        ...
        switch(curr->data) {
	        case <write state>:
				<The follows using transition information>
	    	    if (<write condition> && (curr->CheckParent(<write ith parent>, <write parent>))* ) {
		    	    <write Action>;
        		}
	...
	while (!it1.IsDone()) {
        ...
        switch(curr->data) {
	        case D:
				<The follows using transition information>
	    	    if (<write condition> && (curr->CheckParent(<write ith parent>, <write parent>))* ) {
		    	    <repeat write times according "from" nestedstates>
                    curr = curr->parent;
                    curr = curr->parent;
					:
                    delete curr->left;
					<repeat write times according "to" nestedstates>
