    ...
    public:
        ...
        void OutputCode(ofstream& ofs) {
            ...
            for (unsigned int i = 0; i < BCaseSt.size(); i++) {
                if (!BCaseSt[i][0].empty() || BCaseSt[i].size() > 1) {
                    ofs << "\t\t\t" << "case " << sm.state[i] << ":" << endl;
                    for (unsigned int j = 0; j < BCaseSt[i].size(); j++) {
                        if (!BCaseSt[i][j].empty()) {
                            ofs << BCaseSt[i][j];
                        }
                    }
                    ofs << "\t\t\t\t" << "break;" << endl;
...
private:
    ...
    bool Transition() {
        ...
        if (codeGeneration) {
            ...
            s += string("\t\t\t\t") + "}\n";
            if (priority == 0) {
                _cefsm.cr.BCaseSt[k][priority] += s;
            }
            else {
                if (_cefsm.cr.BCaseSt[k].size() <= (unsigned int)priority) {
                    _cefsm.cr.BCaseSt[k].resize(priority + 1);
                }
                _cefsm.cr.BCaseSt[k][priority] = s;
            }
        }
...
public:
    ...
    bool Parsing() {
        ...
        for (unsigned int i = 0; i < _cefsm.cr.BCaseSt.size(); i++) {
            _cefsm.cr.BCaseSt[i].resize(1);
        }
