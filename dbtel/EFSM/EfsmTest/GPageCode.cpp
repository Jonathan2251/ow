...
void TGPage::ChangeAllConnectTransitions(State *state) {
...
State* TGPage::GetShortestState(int x, int y) {
    State *shortest = 0;
    double distance = UNLIMIT, d;
...
State* TGPage::GetShortestInRangeState(int x, int y) {
    State *shortest = 0;
    double distance = UNLIMIT, d;
    for (int i = 0; i < _stateSize; i++) {
...
Transition* TGPage::GetShortestTransition(int x, int y) {
...
GControl* TGPage::SelectedControl() {
    return _selected;
};

void TGPage::AddState(int left, int top, int right, int bottom) {
...
void TGPage::SelectControl(int x, int y) {
    State *state;
    Transition *tran;
    int d1, d2;
...
void __fastcall TGPage::FormHide(TObject *Sender)
{
    Draw();
    label->Caption = "Hide";
}
