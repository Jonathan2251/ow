...
const char QLcd::Brush() {
	return brush.style;
}

void QLcd::SetBrush(const char Brush) {
	oldBrush = brush.style;
	brush.style = Brush;
}

void QLcd::RestoreBrush() {
	brush.style = oldBrush;
}

void QLcd::DrawLine(const int X1, const int Y1, const int X2, const int Y2, const u16 Color) {
#ifndef QSIMULATE
	if (X1 == X2) { // vertical line
		...
		for(int i = y1 ; i <= y2 ; i++) {
			...
			p_dr_LCD_WRITE(SK43200A_DispMemAcc, Color);
		}
	}
...
void QLcd::DrawRect(const int X, const int Y, const int Width, const int Height, const u16 Color) {
	int y1, y2;
...
#else
	if (brush.style == Qt_NoBrush) {
		DrawLine(X, Y, X+Width-1, Y, Color);
		DrawLine(X, Y+Height-1, X+Width-1, Y+Height-1, Color);
	//	DrawLine(X, Y, X, Y+Height-1, DR_Text_FG_Color);
	//	DrawLine(X+Width-1, Y, X+Width-1, Y+Height-1, DR_Text_FG_Color);
	}
	else if (brush.style == Qt_SolidPattern) {
...
#else
	DrawRect(X, Y, Width, Height, DR_Text_BG_Color);
#endif
}
...
void QWindow::Show() {
	...
	lcd.RestoreBrush();
	if (bordStyle == QWIN_BS_SINGLE) {
		lcd.SetBrush(Qt_NoBrush);
		lcd.DrawRect(out.x, out.y, out.width, out.height, DR_Text_FG_Color);
		lcd.RestoreBrush();
	}
	visible = true;
}
...
void QMultiLineEdit::ShowCursor() {
	if (cursor.type == QCSR_STANDARD) {
		lcd.SetBrush(Qt_SolidPattern);
		lcd.DrawRect(in.x + cursor.p.x, in.y + cursor.p.y, in.x + cursor.p.x + 5, in.y + cursor.p.y, DR_Text_FG_Color);
		lcd.RestoreBrush();
		cursor.on = true;
	}
}
...
//	panel.SetGeometry(1, 16, 99, LINE_HEIGHT);
	panel.Hide();
...
/*	lcd.SetBrush(Qt_SolidPattern);
	lcd.EraseRect(0,0,80,60);
	lcd.RestoreBrush();
	lcd.DrawChar(20, 20, 'w');*/
}
